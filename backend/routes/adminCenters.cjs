...
router.post('/', async (req, res) => {
  ...
  async function ensureId(val) {
    ...
    const [[row] = []] = await pool.query('SELECT id FROM members WHERE username = ?', [val]);
...
router.get('/member-name/:id', async (req, res) => {
  const { id } = req.params;
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: '센터 삭제 실패' });
  }
});
...
router.get('/get-center-owner-name', async (req, res) => {
  ...
  `;
  try {
    const [results] = await pool.query(sql, [center]);
