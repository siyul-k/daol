...
async function updateAllRecommendLineage() {
  const [members] = await connection.query('SELECT id, recommender_id FROM members');
  for (const m of members) {
    ...
    for (let i = 0; i < 15; i++) { // 최대 15대
      if (!current) break;
      ids.push(current);
      const [[row]] = await connection.query('SELECT recommender_id FROM members WHERE id = ?', [current]);
    ...
    await connection.query(`UPDATE members SET ${updateFields} WHERE id = ?`, [m.id]);
  }
